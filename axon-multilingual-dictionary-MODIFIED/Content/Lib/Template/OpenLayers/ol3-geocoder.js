...
Html.prototype.createControl = function createControl () {
  ...
  elements.input.value = this.options.inputText; // 20170212 Axon
  return elements;
};
...
Nominatim.prototype.setListeners = function setListeners () {
  ...
  // 20170212 Axon
  this.els.input.addEventListener('keydown', function(evt){

      if (evt.keyCode == 40) //arrow down
      {
          evt.preventDefault();//prevent window scrolling on Firefox

          var list = this$1.els.result.getElementsByTagName("li");
          if (list.length>0)
          {
              list[0].focus();
          }
  ...
  utils.json(ajax).when({
    ready: function (res) {
      ...
      switch (options.provider) {
        case providers.OSM:
          // FIXME: 20170211 Axon hack: keep only osm_type==="relation"
          //        to be able to request polygon_geojson later on by
          //        querying nominatum with the display_name of the relation.
          var resOsmTypeRelation = [];
          for (var i=0,len=res.length;i<len;i++)
          {
              if (res[i].osm_type==="relation")
                  resOsmTypeRelation.push(res[i]);
          }
          res_ = resOsmTypeRelation.length ?
            this$1.OpenStreet.handleResponse(resOsmTypeRelation) : undefined;
          break;
...
Nominatim.prototype.createList = function createList (response) {
  ...
  {
    var addressHtml = "<span class='search_result'>"+row.original.formatted+"</span>", //20170212 Axon
    ...
    li.addEventListener('keydown', function (e) {
            ...
            e.stopPropagation();//scrolling inside map
            e.preventDefault();//window scrolling
...
Nominatim.prototype.getInput = function inputFocus() {
    var root = DomHelper.getRoot(this.container);
    var inputEl = root.getElementById("gcd-input-query");
    return inputEl;
}
...
var Base = (function (superclass) {
  ...
  //20170212 Axon
  Base.prototype.setInputText = function setInputText (text) {
      var inputEl = this.element.getElementsByClassName("gcd-txt-input")[0];
      inputEl.value=text;
