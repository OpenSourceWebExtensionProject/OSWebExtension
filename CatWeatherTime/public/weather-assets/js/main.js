...
$(document).ready(function () {
    ...
    $.getJSON('./weather-assets/data/data.json?r=' + (Math.random() * 99999), _onDataLoaded);
...
//            }
    ...
    function initUnitToggle(){
        ...
        if (saved_unit == 'F') {
            $c.toggleClass('on', false);
            $f.toggleClass('on', true);
        }
...
//        var icon = $('.sunrise', $icon);
        ...
        $units.toggleClass('night', !getDaytime());
...
//        console.log(w)
      ...
      window.chrome.browserAction.setBadgeText({text: showTemp(weather.temp) + '°'});

    }

    function showTemp(t){
        var unit = localStorage.getItem("bleak_units");
        return (unit=='C') ? String(t) : String(Math.round((t * 1.8) + 32));
    }

    function toggleWeather(){

        var temp = showTemp(weatherData.temp);

        $('h3', $weather).html(temp+ '&deg;');
...
//            console.log(days[d]!=fdays[dayCount])
        ...
        for (i = 0; i < fdays.length; i++) {
            ...
            if (!i) $('.col' + i, $forecast).append("<p class='highlow'>" + showTemp(fdays[i].max) + "</p>");
            else $('.col' + i, $forecast).append("<p class='highlow'>" + showTemp(fdays[i].max) + "/" + showTemp(fdays[i].min) + "</p>");
...
//        console.log("rise", weatherData.sunrise *1000)
//        console.log("set", weatherData.sunset*1000)

        return (time_utc >= weatherData.sunrise*1000 && time_utc < weatherData.sunset*1000);
